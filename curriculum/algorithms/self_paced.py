...
class SelfPaced(BaseCL):
    ...
    def data_prepare(self, loader):
        ...
        self.weights = torch.ones(self.data_size, device=self.device)

        # file path and index mapping
        self.path_to_index = {self.dataset[idx][2]: idx for idx in range(len(self.dataset))}
    ...
    def loss_curriculum(self, criterion, outputs, labels, indices):
        ...
        if isinstance(indices, tuple):
            indices = [self.path_to_index[path] for path in indices]
            indices = torch.tensor(indices, device=self.weights.device)
...
class SelfPacedTrainer(BaseTrainer):
    def __init__(self, data_name, net_name, device_name, num_epochs, random_seed, batch_size, 
                 learning_rate, use_huggingface, 
        ...
        super(SelfPacedTrainer, self).__init__(
            data_name, net_name, device_name, num_epochs, random_seed, batch_size, 
            learning_rate, use_huggingface, cl)